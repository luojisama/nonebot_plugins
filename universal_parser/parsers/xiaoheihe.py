...
class XiaoheiheParser(BaseParser):
    ...
    def __init__(self):
        ...
        if pconfig.xiaoheihe_ck:
            self.headers["cookie"] = pconfig.xiaoheihe_ck
    ...
    async def _parse_bbs_by_link_id(self, link_id: str):
        ...
        if image_urls:
            contents.extend(
                self.create_image_contents(image_urls)
            )
    ...
    def _extract_game_videos(
        ...
        mode = str(getattr(pconfig, "xiaoheihe_video_send_mode", "first") or "first").lower()
        if mode == "none":
            return []
    ...
    def _build_video_content_from_url(
        ...
        if path.endswith(".m3u8"):
            task = DOWNLOADER.download_m3u8(video_url, ext_headers=self.headers)
            return self.create_video_content(task, cover_url)
    ...
    async def _request_json(
        ...
        *,
        ...
        try:
            ...
            raise ParseException(f"???????? JSON: {url}") from exc
