...
class BaseParser:
    ...
    async def parse_with_redirect(
        ...
        """先重定向再解析"""
        redirect_url = await self.get_redirect_url(url, headers=headers or self.headers, use_proxy=self.use_proxy)
    ...
    async def get_redirect_url(
        ...
        """获取重定向后的 URL, 单次重定向"""
        from httpx import AsyncClient, RequestError, ConnectError
        from ..config import pconfig

        headers = headers or COMMON_HEADER.copy()
        proxy = pconfig.proxy if use_proxy else None
