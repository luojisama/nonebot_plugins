...
### 3. AI 绘图 (AI Painting)
- **插件目录**: `plugin/ai_painting`
- **功能**: 支持多模型（OpenAI, SD, Gemini等）的 AI 绘画，包含丰富的预设模板。
- **指令**:
  - `绘图 [描述]`: 基础绘图。
  - `绘图 [图片/@用户] [描述]`: 参考图绘图。
  - `绘图 [N]张 [描述]`: 一次生成多张（最大4张）。
  - `绘图 模板`: 查看所有可用模板（如手办化、拿捏、海的那边等）。
  - `绘图 [模板名] [描述]`: 使用指定模板绘图。
  - `查询余额` / `ai余额`: 查看绘图 API 的剩余额度。
  - `绘图白名单 添加/删除 [群号]`: (超级用户) 设置群组免费绘图。
  - `绘图帮助`: 查看详细用法。
- **主要配置**:
  | 配置项 | 类型 | 默认值 | 说明 |
  | :--- | :--- | :--- | :--- |
  | `ai_painting_api_key` | `str` | (必填) | API Key |
  | `ai_painting_model` | `str` | `nano-banana-2-2k` | 绘图模型 |
  | `ai_painting_price` | `int` | `8` | 每张图消耗的金币 |
...
### 5. LoveLive! 抽卡 (LoveLive Gacha)
- **插件目录**: `plugin/lovelive_gacha`
- **指令**:
  - `ll单抽 [缪斯/水团/虹咲/星团]`: 进行一次单抽。
  - `ll十连 [缪斯/水团/虹咲/星团]`: 进行一次十连抽（保底SR）。
...
### 3. LoveLive! 日程 (LoveLive Schedule)
- **插件目录**: `plugin/lovelive_schedule`
- **功能**: 定时爬取 LoveLive! 活动及声优访华日程。
- **指令**:
  - `ll日程` / `ll全部日程`: 查看最新活动日程。
  - `ll访华`: 查看声优访华计划。
  - `ll开启推送` / `ll关闭推送`: (管理员) 开启或关闭本群的日程自动推送。

### 4. 视频/链接解析 (Universal Parser)
- **插件目录**: `plugin/universal_parser`
- **功能**: 自动解析并下载 B站、抖音、微博、YouTube 等平台的视频或图文。
- **指令**:
  - 直接发送支持的链接即可自动触发解析。
  - `ym [URL]`: 下载 YouTube 视频音频。

### 5. 其他实用工具
| 插件 | 指令 | 说明 |
| :--- | :--- | :--- |
| **网页快照** | `/截图 [URL]` | 将指定网页渲染为全屏长图发送 |
| **日常工具箱**| `epic` / `喜加一` | 本周 Epic 免费游戏信息 |
| | `疯狂星期四` / `kfc` | 随机 KFC 骚话文案 |
| | `冷笑话` / `讲个笑话` | 讲一个冷笑话 |
| | `发病 [@用户]` | 生成针对某人的发病文案 |
| | `黄金价格` / `金价` | 获取今日实时金价信息 |
| | `健康分析 [身高] [体重] [年龄] [性别]` | 获取身体健康深度分析 |
...
# 绘图额外配置
AI_PAINTING_BALANCE_TOKEN="xxx"
AI_PAINTING_BALANCE_USER_ID="xxx"
...
> **注意**: 部分插件（如 CS Pro、网页快照、日程）需要环境中安装 `Playwright` 浏览器内核。如果无法运行，请执行 `playwright install chromium`。
